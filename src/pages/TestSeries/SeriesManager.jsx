...
const EMPTY_FORM = {
  title: '', description: '', price: '', mrp: '', duration: '',
...
const getError = (error, fallback) => error?.response?.data?.message || error?.response?.data?.error || error?.response?.data || error?.message || fallback
...
export default function SeriesManager() {
  ...
  const submit = async (event) => {
    ...
    ;['price', 'mrp', 'duration'].forEach((key) => { values[key] = values[key] === '' ? null : Number(values[key]) })
      ...
      <div className="card series-table-card">
        <div className="table-wrap">
          <table className="data-table series-table">
            <thead><tr><th>ID</th><th>Name</th><th>Exam Type</th><th>Duration</th><th>Price</th><th>MRP</th><th>Category</th><th>Features</th><th>Description</th><th>Active</th><th>Created At</th><th>Image</th><th>Actions</th></tr></thead>
            <tbody>
              ...
              {!loading && filtered.map((row) => (
                <tr key={row.id}>
                  <td>{row.id}</td><td>{row.title || '—'}</td><td>{row.subject || row.examType || '—'}</td><td>{row.duration ?? '—'}{row.duration ? ' min' : ''}</td><td>{row.price ?? '—'}</td><td>{row.mrp ?? '—'}</td>
                  ...
                  <td>{row.image ? <img className="series-thumb" src={row.image} alt="" /> : <FiImage />}</td>
      ...
      {showModal && <Modal title={editing ? 'Edit Test Series' : 'Create Test Series'} onClose={closeModal} maxWidth="980px" footer={<><button className="btn btn-outline" type="button" onClick={closeModal}>Cancel</button><button className="btn btn-primary" type="submit" form="series-form" disabled={saving}>{saving ? 'Saving…' : editing ? 'Update' : 'Create'}</button></>}>
        ...
        <form id="series-form" onSubmit={submit} className="series-form">
          <div className="series-form-grid">
            ...
            <div className="form-group"><label htmlFor="series-duration">Duration</label><input id="series-duration" name="duration" type="number" min="0" value={form.duration} onChange={change} placeholder="Minutes" /></div>
